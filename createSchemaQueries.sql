...
alter table Students add column campus varchar(10);
...
CREATE TABLE Project(
	...
	description TEXT,
	projectName VARCHAR(255) DEFAULT NULL,
    projectDomain VARCHAR(30) DEFAULT NULL,
    proposalFile LongBlob DEFAULT NULL,
	startDate Date,
	status ENUM('Not Started','In Progress','Completed') DEFAULT "Not Started",
	createdAt timestamp default CURRENT_TIMESTAMP,
	updatedAt timestamp default current_timestamp ON UPDATE CURRENT_TIMESTAMP
...
select t.* from fypStudent f join projectgroup pg on f.groupID=pg.groupID join supervisor s on pg.supervisorID = s.supervisorID join teachers t on s.supervisorID=t.teacherID where f.fypStudentID=23;


select t.* from fypStudent f join projectgroup pg on f.groupID=pg.groupID join supervisor s on pg.supervisorID = s.supervisorID join teachers t on s.supervisorID=t.teacherID where f.fypStudentID=16;
...
SELECT 
  ...
  where p.supervisorID = 17;
  
  
  update proposal
  set proposalStatus=0
